...
KIBAR_STATUS KIBAR_I2C_WRITE_VALUE_ONE_BIT_TO_REGISTER(I2C_HandleTypeDef* kibar_i2c, uint8_t shift_value, uint8_t device_address, uint8_t register_address) {
	...
	HAL_StatusTypeDef statusone = HAL_I2C_Mem_Read(kibar_i2c, device_address, register_address, KIBAR_REGISTER_GENERAL_SIZE_, &register_data, KIBAR_REGISTER_GENERAL_SIZE_, KIBAR_TIMEOUT_);
	if(statusone != HAL_OK) return kibar_ERROR;
	register_data = register_data | (0x01 << shift_value);
	statusone = HAL_I2C_Mem_Write(kibar_i2c, device_address, register_address, KIBAR_REGISTER_GENERAL_SIZE_, &register_data, KIBAR_REGISTER_GENERAL_SIZE_, KIBAR_TIMEOUT_);
	if(statusone != HAL_OK) return kibar_ERROR;
